...
import {
    Box, Grid, Card, CardHeader, CardContent, TextField, Select, MenuItem,
        ...
        <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
                <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 3 }}>
                        <FormControl fullWidth>
                            ...
                            >
                                {Object.entries(groups).map(([group, groupKeys]) => (
                                    groupKeys.length > 0 && [
                                        <ListSubheader key={group} sx={{ bgcolor: 'background.paper', fontWeight: 'bold' }}>{group}</ListSubheader>,
                                        ...groupKeys.map(key => (
                        ...
                        {variables.length > 0 && (
                            <Box>
                                <Typography variant="caption" display="block" sx={{ mb: 1 }}>Variables Detected:</Typography>
                                <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                                    {variables.map(v => (
                                        <Chip key={v} label={v} size="small" variant="outlined" sx={{ fontFamily: 'monospace' }} />
                                    ))}
                                </Box>
                            </Box>
                        )}
            ...
            <Grid item xs={12} md={6}>
                <Card sx={{ height: '100%' }}>
                    <CardHeader
                        title="Live Preview"
                        action={<Chip label="WHATSAPP STYLE" color="success" size="small" />}
                        sx={{ borderBottom: 1, borderColor: 'divider' }}
                    />
                    <Box sx={{ p: 0 }}>
                        <div className="whatsapp-preview">
                            ...
                            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0.75rem', background: '#1f2c34', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                                ...
                                <div style={{ flex: 1, height: 36, background: '#2a3942', borderRadius: 18, padding: '0 1rem', display: 'flex', alignItems: 'center', color: '#8696a0', fontSize: '0.9rem' }}>Type a message</div>
                        ...
                        <CardContent>
                            ...
                            <Typography variant="subtitle2" sx={{ mb: 1 }}>Test Actions</Typography>
                            <Grid container spacing={1}>
                                <Grid item xs={6}>
                                    <Button variant="outlined" fullWidth onClick={() => triggerTest('morning')} startIcon={<Sun size={16} />}>
                                        Test Morning
                                    </Button>
                                </Grid>
                                <Grid item xs={6}>
                                    <Button variant="outlined" fullWidth onClick={() => triggerTest('evening')} startIcon={<Moon size={16} />}>
                                        Test Evening
                                    </Button>
                                </Grid>
                            </Grid>
