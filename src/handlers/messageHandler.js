...
4️⃣ *!ingatkan* (Tag Manual)
5️⃣ *!setgroup* (Set Lokasi Alarm)`;
        ...
        if (command === '!setgroup') {
            if (!isGroup) {
                await sock.sendMessage(sender, { text: '❌ Perintah ini harus di dalam Grup.' }, { quoted: msgObj });
                return;
            }

            // Simpan ID Grup ke file
            fs.writeFileSync(GROUP_ID_FILE, sender);
            await sock.sendMessage(sender, { text: `✅ *GRUP TERSIMPAN!* \n\nAlarm Otomatis (Jam 18, 20, 22) akan dikirim ke grup ini.` }, { quoted: msgObj });
            return;
        }
        ...
        if (command === '!ingatkan') {
            if (!isGroup) {
                await sock.sendMessage(sender, { text: '❌ Khusus Grup.' }, { quoted: msgObj });
                return;
            }

            const allUsers = getAllUsers();
            if (allUsers.length === 0) {
                await sock.sendMessage(sender, { text: '⚠️ Belum ada user terdaftar.' }, { quoted: msgObj });
                return;
            }
